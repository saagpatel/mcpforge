...
Latest local result on 2026-05-10: the baseline above passed, `uv build` produced the `0.2.0` source distribution and wheel, `uv run pytest` reported 300 passed and 2 skipped, CLI help/version/list smokes passed, all five Python example server test files passed when run from their own example projects, the TypeScript todo example validated from a temporary copy with 2 tests run and 0 failed, and hosted Python plus TypeScript generation passed with `ANTHROPIC_API_KEY` loaded from Keychain.
