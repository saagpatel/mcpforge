...
| --- | --- | --- | --- |
| Local package and validation | Green | `fastmcp-builder==0.3.4` is published, `mcpforge version` reports `0.3.4`, and the safe local verification commands below are the live validation baseline. | None for local-only work. |
...
uvx --from fastmcp-builder==0.3.4 mcpforge version --json
```
